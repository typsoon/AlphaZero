...
def inference_server():
    ...
    p = subprocess.Popen([executable, "--socket", SOCKET_PATH, "--network-path", model_path, "--device", "cpu"])
...
def game_server(inference_server):
    ...
    p = subprocess.Popen(["python", "python/game_server.py", "--socket", SOCKET_PATH, "--port", str(PORT)], env=env)
    
    for _ in range(50):
        try:
            resp = requests.get(f"http://localhost:{PORT}/game/status", headers={"Connection": "close"})
...
def test_game_server_e2e(game_server):
    status_resp = requests.get(f"http://localhost:{PORT}/game/status", headers={"Connection": "close"})
    assert status_resp.status_code == 200
    
    move_resp = requests.post(f"http://localhost:{PORT}/game/move", json={"column": 3}, headers={"Connection": "close"})
