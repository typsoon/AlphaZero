...
void set_up_and_run_server(std::string socket_path, std::shared_ptr<M> model,
    ...
    CROW_ROUTE(app, "/predict")
        .methods(crow::HTTPMethod::Post)(
            [model, schema_validator](const crow::request &req, crow::response &resp) {
                try {
                    const auto request_json = nlohmann::json::parse(req.body);
                    if (!request_json.is_object() || !request_json.contains("game_state")) {
                        resp.code = 400;
    ...
    CROW_ROUTE(app, "/health")
        .methods(crow::HTTPMethod::Get)([](const crow::request &, crow::response &resp) {
            resp.code = 200;
            resp.set_header("Content-Type", "application/json");
...
template void
    set_up_and_run_server<ModelWrapper, SchemaValidator>(std::string, std::shared_ptr<ModelWrapper>,
