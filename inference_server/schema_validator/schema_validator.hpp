...
class SchemaValidator {
    ...
    friend std::shared_ptr<SchemaValidator> get_schema_validator(std::filesystem::path schema_path);

  public:
    bool is_a_valid_boardstate(std::string body);
};

std::shared_ptr<SchemaValidator> get_schema_validator(std::filesystem::path schema_path);
