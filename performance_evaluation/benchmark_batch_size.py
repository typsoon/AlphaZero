...
def run_benchmark(benchmark_binary, network_path):
    ...
    result = subprocess.run(
        [benchmark_binary, network_path], check=True, capture_output=True, text=True
    )
    ...
    # Parse output, e.g.: Batch 32: 1.4834 ms latency, 0.123 ms memory transfer, 21572.1 evals/sec
    pattern = re.compile(
        r"Batch\s+(\d+):\s+([\d.]+)\s+ms latency,\s+([\d.]+)\s+ms memory transfer,\s+([\d.]+)\s+evals/sec"
    )
    for line in result.stdout.splitlines():
        ...
        if match:
            ...
            mem_latencies.append(float(match.group(3)))
            evals_per_sec.append(float(match.group(4)))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--inference-binary",
        required=True,
        help="Path to inference_server binary (used to find the benchmark binary)",
    )
    parser.add_argument(
    ...
    if not benchmark_bin.exists():
        print(
            f"Benchmark binary not found at {benchmark_bin}. Did you build the project?"
        )
        sys.exit(1)
